...
void CMFCApplication3Dlg::OnPaint()
{
	...
	else
	{
		...
		for (int dan = 2; dan <= 9; dan++) {
			for (int i = 1; i <= 9; i++) {
				str.Format(L"%d X %d = %d", dan, i, dan*i);
				x = 10; y = 5 + 24 * count;
				dc.TextOut(x, y, str);
				count++;
			}
...
void CMFCApplication3Dlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch (nSBCode) {
	case SB_LINEUP:
		if(min<scroll_pos)	SetScrollPos(SB_VERT, --scroll_pos);
		break;
	case SB_LINEDOWN:
		if (max > scroll_pos)	SetScrollPos(SB_VERT, ++scroll_pos);
		break;
	case SB_PAGEUP:
		if (min < scroll_pos+24)	SetScrollPos(SB_VERT, scroll_pos -24);
		break;
	case SB_PAGEDOWN:
		if (max> scroll_pos-24)	SetScrollPos(SB_VERT, scroll_pos +24);
		break;
