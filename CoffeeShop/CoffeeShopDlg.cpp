...
BOOL CCoffeeShopDlg::OnInitDialog()
{
	...
	wchar_t *p_item_list[MAX_ITEM_COUNT] = {
		L"아메리카노	  1900원", L"카페라떼		  2500원",
		L"카페모카		  2800원", L"카라멜마키아또   3200원",
	...
	for (int i = 0; i < 8; i++) {
		m_item_list.InsertString(i, p_item_list[i]);
...
void CCoffeeShopDlg::OnLbnSelchangeItemList()
{
	...
	for (int i = 0; i < count; i++) {
		if (m_item_list.GetCheck(i))	total_price += m_item_list.GetItemData(i);
	}
